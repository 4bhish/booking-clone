...
const stayOptions = [
  {
    name: "Book a Stay",
    description: "Get a better understanding of yout traffic",
    href: "#",
    icon: HomeIcon,
  },
  {
      ...
      >
        <div className="flex lg:flex-1">
          <Link href={"/"} className="-m-1.5 p-1.5">
            <span className="sr-only">Booknow.com</span>
            <img
                ...
                <div className="grid grid-cols-2 divide-x divide-gray-900/5 bg-gray-50">
                  {callToAction.map((item) => (
                    <a
                      key={item.name}
                      href={item.href}
                      className="flex items-center justify-center gap-x-2.5 p-3 text-sm font-semibold  text-[#013B94] hover:bg-gray-100"
                    >
                      <item.icon
                  ...
                  {({ open }) => (
                    <>
                      <Disclosure.Button className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base  font-semibold  leading-7 text-white hover:bg-blue-800">
                        Stays
                        <ChevronDownIcon
                          className={cn(open ? "rotate-180" : "", "w-5 h-5")}
                        />
                      </Disclosure.Button>
                      <Disclosure.Panel className="mt-2 space-y-2 ">
                        {[...stayOptions, ...callToAction].map((item) => (
                          <Disclosure.Button
                            key={item.name}
                            as="a"
                            href={item.href}
